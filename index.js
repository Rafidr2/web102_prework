...
// calculate the total amount of money raised across all games
const totalRaised = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);

// create a string that explains the number of unfunded games using the ternary operator
const descriptionStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} game${totalGames === 1 ? '' : 's'}. Currently, ${unfundedGames} game${unfundedGames === 1 ? ' remains' : 's remain'} unfunded. We need your help to fund these amazing games!`;
...
// Create and append a new element with the name of the top-funded game
...
// Create and append a new element with the name of the second-most funded game
const secondTopGameNameElement = document.createElement('h3');
...
donationForm.addEventListener("submit", function(event) {
    ...
    if (email) {
        formMessage.textContent = `Thank you! We will reach out to you at ${email}.`;
